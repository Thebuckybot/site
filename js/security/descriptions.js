...
export const SETTING_DESC = {
  ...
  recovery: "Restores the server's structure from a trusted snapshot. Safe Repair recreates what's missing and never deletes; Full Restore can also remove resources that are absent from the snapshot, after an explicit confirmation. Detection ignores Bucky's own rebuild (self-action guard).",
  ...
  rollback_action: "Opens the Restore console for the newest trusted snapshot. Choose Safe Repair (recreate missing structure — never deletes) or Full Restore (also delete resources absent from the snapshot, after an explicit typed confirmation). You preview the exact changes before anything runs. Detection ignores Bucky's own rebuild (self-action guard). Use AFTER containment, not during.",
...
export const STAGE_DESC = {
  ...
  snapshot_rollback: "Automatically recreate channels/roles that are missing versus the latest trusted snapshot (Safe Repair — never deletes). For nuke recovery; only valid on structural modules.",
